...
or a filename?  One way is if it begins with C<_TINY_XML_STRING_> then it's
...
to pass in a string would be expected to use L<IO::Scalar>.  Alternatively,
pass a true value for the parameter input_is_string.
...
=item preserve_whitespace

If set to true, do not delete text nodes which consist of nothing but whitespace.
    
=item input_is_string

If true, treat the first parameter as a string of XML, rather than a filename.
...
sub parsefile {
    ...
    if($params{input_is_string}) {
        $file = $arg
    }
    elsif(ref($arg) eq '') { # we were passed a filename or a string
    ...
    # This is rubbish -- changes >  to &gt; in text
...
#     while($file =~ s/($regexps{name}\s*=\s*'[^']*)>([^']*')/$1&gt;$2/gsi) {}
    ...
    foreach my $token (grep { length }
      split(/(<[^>]+>)/, $file)) {
	    ...
	    next;
        } elsif ($token =~ /^\s+$/ && !$params{preserve_whitespace}) { # whitespace tokens
            next;

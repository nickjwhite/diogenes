...
# This is the directory whence the pointless decorative images that
# come with the script are served.  It can safely be the same
# directory as $img_dir above.
...
{
    my $self = shift;
    print '<center><div style="display: block; width: 50%; text-align: left;">
<p><center><strong>Diogenes ERROR</strong></center></p>';
    
    if (not $self)
    {
        # Diogenes is just starting up
        print '<p><center>No TLG, PHI or DDP database was found.</center></p>

<p> I looked in the places CD-Roms are usually found, and did
not find anything.</p>

<p> Have you remembered to put the CD-Rom in the drive?
Sometimes it takes a few seconds after a disk has been inserted for
the CD-Rom drive to spin up to speed, so you might want just to hit
the reload button to try again.</p>';

    }
    else
    {
        print '<p>The requested database was not found.</p>';

        print '<p>I looked here: ' . $self->{cdrom_dir} . '</p>' if $self->{cdrom_dir};

        print'
<p> Have you remembered to put the correct CD-Rom in the drive?
Sometimes it takes a few seconds after a disk has been inserted for
the CD-Rom drive to spin up to speed, so you might want just to hit
the back button and try your search again.</p>';
    }

    print'<p>If that does not work and you are sure that there is a
CD-Rom present, then it may be that your computer is configured in an
unusual manner.  So the next thing to try is to tell Diogenes
explicitly where to find your CD-Rom.</p>

<p>You can do this by clicking on this link to the <a
href="Settings.cgi">Settings Page</a>.  There you can specify the
location of each database by hand.  If your databases are located
somewhere other than on a CD-Rom, such as on a network, you will have
to specify that location on the <a href="Settings.cgi">Settings
Page</a></p>';
...
my $print_page_3 = sub 
{
    ...
    $database_error->($query) if not $query->check_db;
...
elsif ($ENV{diogenes_no_databases} and not $init->check_cdroms)
{
    print $form->start_html(-title   =>'Diogenes Error Page',
                            -bgcolor =>'#FFFFFF',
                            -text        =>"#000000", 
...
{
    delete $ENV{diogenes_no_databases};

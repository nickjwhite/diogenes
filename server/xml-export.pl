...
getopts ('alprho:c:sn:vdetl');
our ($opt_a, $opt_l, $opt_p, $opt_c, $opt_r, $opt_h, $opt_o, $opt_s, $opt_v, $opt_n, $opt_e, $opt_t, $opt_x);
...
if (not $libxml) {
    ...
    if ($success) {
        XML::DOM::Lite->import( qw(Parser Serializer :constants) );

...
my @tlg_files = 
('authtab.dir', 'tlgwlinx.inx', 'tlgwlist.inx', 'tlgwcinx.inx', 'tlgwcnts.inx', 'tlgawlst.inx');

{	# For closure
						# Default values for all Diogenes options.
						# Overridden by rc files and constructor args.

	my %defaults = (	type => 'phi',
						output_format => 'ascii',
						highlight => 1,
						printer => 0,
						input_lang => 'l',
...
### ## inserted by P A MacKay 
# A useful check to see what the ultimate environment is, and why
#     print STDERR "key = value pairs in defaults\n";
#     while ((my $selfkey, my $selfstr) = each %defaults) {
#         printf STDERR "%s = %s\n", $selfkey, $selfstr;
#     }
#     print STDERR "key = value pairs in rc_defaults\n";
#     while ((my $selfkey, my $selfstr) = each %rc_defaults) {
#         printf STDERR "%s = %s\n", $selfkey, $selfstr;
#     }
#     print STDERR "key = value pairs in args\n";
#     while ((my $selfkey, my $selfstr) = each %args) {
#         printf STDERR "%s = %s\n", $selfkey, $selfstr;
#     }
#     print STDERR "key = value pairs accumulated in self\n";
#     while ((my $selfkey, my $selfstr) = each %{ $self }) {
#         printf STDERR "%s = %s\n", $selfkey, $selfstr;
#     }
		...
		elsif ($self->{type} eq 'cop') 
		{
			...
			$self->{coptic_encoding} = 'beta' if $args{output_format} and $args{output_format} eq 'beta';
		}
		...
		{
			$self->{$_} = uc $self->{$_} for qw(file_prefix txt_suffix cdrom_dir idt_suffix);
		}
		...
		die "Illegal value for context: $self->{context}\n" unless $self->{context} =~ m/^(?:sentence|paragraph|clause|phrase|\d+\s*(?:lines)?)$/;
...
{	# Set up links to Perseus morphological parser 
	...
	$self->{perseus_morph} = 1 if $self->{perseus_links} and $self->{output_format} =~ m/html/; 
	$self->{perseus_morph} = 0 if $self->{type} eq 'cop';
	$self->{perseus_morph} = 0 if $self->{encoding} =~ m/babel/i;
}	

# Restricts the authors and works in which to search according to the settings
# passed, and returns those authors and works.
sub select_authors 
{
	my $self = shift;
	my %passed = @_;
	...
	$args{'criteria'} = ((keys %args) - 1) if $args{'criteria'} =~ m/all/i; #the 1 is 'criteria' itself
...
sub parse_lists 
{
	my $self = shift;
	my @tlg_class_files = 
		( qw(list3cla.bin list3clx.bin list3dat.bin list3epi.bin list3fem.bin list3geo.bin) );
...
sub beta_to_beta
{
	...
	$begin = 1 if $pat =~ s#^\s+##;
	$end   = 1 if $pat =~ s#\s+$##;
	($pat, undef) = Diogenes_indexed::make_tlg_regexp($self, $pat, (not $begin), (not $end));
	return $pat;
}
...
sub latin_pattern
{
	...
	# Add non-alphabetics
	$pat =~ s/(\[[^\]]*\][*+?]?)(?!$)/$1(?:-\[$non_alpha_with_space\]\*|\[$non_alpha\]\*)/g;
...
#	s/\s+$/(?=[^A-Z])/g;  		# spaces at the end -- this doesn't work -- previous glob backtracks off
    ...
    s#¨¨#(?=\\\(|[AEHIOWU/\\\\)=+?!|']+\\\()#gi;    # h looks ahead for a rough breathing 
                                                    # somewhere in the following group of vowels,
                                                    # or even before it as in the word (EA`N
		...
		{	# new author or work
			...
			if (ord (substr ($idt_buf, ++$i, 1)) == hex ("ef")) 
			{
				...
				{	# Trap this for now
					...
					{ 
						$self->barf("Sub-work number apparently was not followed by 
						sub-work name in idt file $file")
					}
				}
				else 
				{
					$self->barf ("I don't understand level $lev after \0xef in idt
					file $file.")
				}
			}
			else 
...
sub get_tlg_biblio_info
{
	...
	{
		while 
		($info =~ m/[\x80-\x8f]?$field ([\x00-\x7f]*(?:[\x80-\x8f]    [\x00-\x7f]+)*)?\s?[\x80-\x8f]?/g)
...
HIT: foreach $match ( 0 .. $#{ $self->{seen}{$auth} } ) 
	 {
		...
		# Optimize case where one hit comes close after the previous, so as not to go back 
		# to parse once again unneccessarily from the beginning of the same block.
		# But watch out for cases where a failed multiple match (next HIT) has skipped that parsing
		if ($parsed_block and $last_offset != -1 and ($offset - $last_offset < $offset - $buf_start))
		{
...
#			print STDERR "+ ".$self->{pattern_list}[0]."\n" if $self->{debug};
		...
		# Set flag to true, since we have passed by all of the next HIT statements, and 
		# we have definitely parsed the location info for this hit, meaning we can use it as the
		# basis for the location info for the next hit in this block
		$parsed_block = 1;
...
#print STDERR ">>".$self->{type}.$self->{auth_num}."||\n";
		...
		$this_work .= "$Diogenes::work{$self->{type}}{$self->{auth_num}}{$self->{work_num}} ";
		...
		{
			if ($Diogenes::level_label{$self->{type}}{$self->{auth_num}}{$self->{work_num}}{$_})
			{
				$location .=
...
{ 	
	...
	{	# num, then string
		...
		if ($self->{documentary})
		{
			# Nasty hack for when this string contains stuff like 1[3], which could
			# be BETA formatting code, but isn't (see Fouilles de Delphes in the Cornell
...
sub format_output
{
    ...
    $self->{perseus_morph} = 0 if ($Diogenes::encoding{$self->{encoding}}{remap_ascii});
        ...
        # Get rid of null chars.  We can't do this last, as we would like, because this represents a grave
        # accent for many encodings (e.g. displaying Ibycus via HTML).  We have to leave something here as a marker
        # or formatting gets confused.  So all formats must remember to remove this string.
        $$ref =~ s/\`/ˇÆˇ/g;
...
# output PAM 090102
        ...
        if ($self->{output_format} eq 'html')
    {
        ...
        {
            # This is a Greek font that remaps the ascii range, and so it is almost certainly
            # not safe to parse the output as Beta, since the Greek encoding will contain HTML
            # Beta control and formatting chars.  So we just escape the HTML codes, and send it 
            # as-is
...
sub perseus_handler
{
        ...
        while ($$ref =~ m/([$beta$punct\d]*)([^$beta]*)/g)
        {
                ...
                {
                        $link =~ s/[$punct\d]//g;
                        $link =~ tr/A-Z/a-z/ if $lang eq 'greek'; # Perseus morph parser takes Beta, but lowercase
                        ...
                        $link =~ s/([^a-zA-Z0-9_.-])/'ˇˇ'.uc sprintf("%%%02x",ord($1))/eg; # URL escape (from CGI.pm)
...
sub utf8_to_beta_encoding
{
        ...
        my %accents = ( '225' => 'a/', '233' => 'e/', '237' => 'i/', '243' => 'o/', '250' =>'u/', 
...
sub beta_to_latex 
{
        ...
        my @gk_font = ('', '\fontseries{b}', '',
        '\fontshape{sl}','','','','','', '', "\\fontsize$small",
        "\\fontsize$small\\fontseries{b}",
        '',"\\fontsize$small\\fontshape{sl}",'','','','','','',"\\fontsize$large",
        "\\fontsize$large\\fontseries{b}", '', "\\fontsize$small\\fontshape{sl}");
        my @rm_font = ('', '\fontseries{bx}', '',
        '\fontshape{it}','','','\fontshape{sc}','\fontshape{sc}','\fontshape{sc}','',
        "\\fontsize$small",
        "\\fontsize$small\\fontseries{bx}",
        '',"\\fontsize$small\\fontshape{it}",'','','','','','',"\\fontsize$large",
        "\\fontsize$large\\fontseries{bx}", '', "\\fontsize$small\\fontshape{it}");
        
        my @sym = ('', '{\greek{k+}}', '\ensuremath{\cdot}', '{\greek{k+}}', '', 
        '{\greek{s+}}',
        ...
        # If we are generating postscript or PDF for portability, we don't want any 
        # bitmapped fonts
...
{       
        ...
        my $start_pat = 0;
        $start_pat++ if $pattern =~ s#¨¨#(?=\\\(|[AEHIOWU/\\\\)=+?!|']+\\\()#gi;		# h looks ahead for a rough breathing 
        $pattern =~ s#¨£#(?!\\\(|[AEHIOWU/\\\\)=+?!|']+\\\()#gi;		# non-rough breathing (possibly not at word beginning) 
        ...
        return if $Diogenes_Daemon::flag and not print ("\0");
                ...
                unless ($block_num == $current_block and $word_num == $current_word_num + 1)
                {
                        ...
                        for ($i = 1; $i < $word_num; $i++) 
                        {
                                ...
                                seek WCNTS, $entry_length, 1 or $self->barf("Couldn't seek through $tlgwcnts");
                        }
                }
...
WORK:   foreach $work (sort keys %{ $self->{word_counts}{$author} }) 
                {
                        ...
                        else
                        {
                                ...
                                my $next = "$work"; 
                                ++$next;
                                ++$next until exists ($Diogenes::work_start_block{$self->{type}}{$author}{$next});
                                $end_block = $Diogenes::work_start_block{$self->{type}}{$author}{$next};
                    ...
                    return if $Diogenes_Daemon::flag and not print ("\0");
                                                        ...
                                                        ($self->{hits_hash}{$author}{$work}{$word_key} || 0) )
                                        {
                                                ...
                                                {       # Maximus Confessor
                                                        warn "Word list counts are off for author $author, work $work.  " .
...
sub browse_location 
{
        ...
        foreach $lev (sort numerically keys %{ $Diogenes::level_label{$self->{type}}{$real_num}{$work} }) 
        {
...
SECTION:
    for (@{ $Diogenes::top_levels{$self->{type}}{$auth}{$work} })
    {
        ...
        if (compare(@{ $_ }[0], $target{$top_level}) > 0)
        {
            ...
            print STDERR ">>>@{ $_ }[0] => $target{$top_level} (using block $old_block)\n" if $self->{debug};
            $block = $old_block;
            last SECTION;
        }
        $old_block = $block;
...
LEVEL:
        ...
        {   
            # See below
                next LEVEL if $Diogenes::level_label{$self->{type}}{$auth}{$work}{$lev} =~ m#^\*#;
                next LEVEL unless $target{$lev};
            my $result = compare($level->{$lev}, $target{$lev});
            print STDERR "¨¨¨$level->{$lev} <=> $target{$lev}: res = $result ($lev, $cite_block)\n" if $self->{debug};
            if ($result == 0)
            {
                next LEVEL;
            }
...
#############################################################################
#                                                                                                                                                       #
# As above, except with CGI scripts we have to re-read the text, since we       #
# don't want to hold it in memory between invocations.                                          #
#                                                                                                                                                       #
#############################################################################
